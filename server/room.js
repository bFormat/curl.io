...
class Room {
  ...
  handleInput(player, msg) {
    ...
    if (player.inputQueue.length < 240) player.inputQueue.push(msg);
  }
  ...
  applyInputs(dt) {
    for (const p of this.players.values()) {
      ...
      const inputs = p.inputQueue
        .filter((i) => i.seq > p.lastInputSeq)
        .sort((a, b) => a.seq - b.seq);
      p.inputQueue.length = 0;
      if (inputs.length) {
        for (const inp of inputs) {
          Sim.step(p, inp, typeof inp.dt === 'number' ? inp.dt : dt, MAP);
          p.lastInputSeq = inp.seq;
        }
      } else {
        // 입력 없어도 중력/넉백 진행
        Sim.step(p, { move: {}, jump: false, yaw: p.yaw, pitch: p.pitch }, dt, MAP);
      }
    }
  }
  ...
  applyFire(now) {
    for (const p of this.players.values()) {
      ...
      for (const f of p.fireQueue) {
        const yaw = typeof f.yaw === 'number' ? f.yaw : p.yaw;
        const pitch = typeof f.pitch === 'number' ? f.pitch : p.pitch;
        if (f.slot === 'primary') {
  ...
  castSkill(p, skillId, now) {
    ...
    if (skillId === 'repulse') {
      ...
      this.events.push({ kind: 'skill', skill: 'dash', id: p.id });
    } else if (skillId === 'bearing') {
      p.buffs.sniperBearings = { charges: def.charges };
      this.events.push({ kind: 'skill', skill: 'bearing', id: p.id });
    }
  }
